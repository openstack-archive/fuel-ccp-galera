...
def set_globals():
    ...
    MYSQL_ROOT_PASSWORD = six.moves.shlex_quote(config['db']['root_password'])
    CLUSTER_NAME = six.moves.shlex_quote(config['percona']['cluster_name'])
    XTRABACKUP_PASSWORD = six.moves.shlex_quote(config['percona']
                                                      ['xtrabackup_password'])
    MONITOR_PASSWORD = six.moves.shlex_quote(config['percona']
                                                   ['monitor_password'])
...
def mysql_init():
    ...
    mysqld_proc = execute_cmd("mysqld --skip-networking")
