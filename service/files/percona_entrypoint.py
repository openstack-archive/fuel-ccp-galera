...
def run_step_one(etcd_client, lock, ttl):
    ...
    """
    In this step we're making bootstrap preparations. We fetching current
    cluster state, registering ourselfs in the queue, if we in the recovery
    situation, we need to get our seqno from mysql, after that we done, so we
    fall into the endless loop waiting 'till other nodes do the same and after
    that we wait for our turn, based on the ctime, to start jointing the
    cluster.
    """
...
def run_step_two(etcd_client, lock, ttl):
    ...
    If the queue list is empty and nodes list is equals to 3, we assume that
    we're the last one. In the one last case we're the second one.
    ...
    not by registration ctime, but by the seqno of their data, so first one
    node will the one with the most recent data.
    """
...
def run_step_three(etcd_client, first_one, last_one):
...
def main(ttl):
    ...
    try:
        ...
        if state is None or state == 'BUILDING':
            ...
            run_step_three(etcd_client, first_one, last_one)
